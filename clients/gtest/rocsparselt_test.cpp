 ...
 * callback function
 *********************************************/
thread_local std::unique_ptr<std::function<void(rocsparselt_handle)>> t_set_stream_callback;
...
extern "C" void rocsparselt_test_signal_handler(int sig)
{
...
void rocsparselt_test_sigaction()
{
    ...
    act.sa_handler = rocsparselt_test_signal_handler;
    ...
    for(int sig : {SIGABRT, SIGFPE, SIGILL, SIGINT, SIGSEGV, SIGTERM})
        signal(sig, rocsparselt_test_signal_handler);
#endif
}
        ...
        for(size_t j = 0; j < streams; ++j)
        {
            if(numStreams)
                t_set_stream_callback.reset(
                    new std::function<void(rocsparselt_handle)>([=](rocsparselt_handle handle) {
...
bool match_test_category(const Arguments& arg, const char* category)
{
    ...
    if(*arg.known_bug_platforms)
    {
        ...
        static const std::string platform = rocsparselt_internal_get_arch_name();
