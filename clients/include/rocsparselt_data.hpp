...
class RocSparseLt_TestData
{
    // data filename
    static auto& filename()
    {
        static std::string filename
            = "(Uninitialized data. RocSparseLt_TestData::set_filename needs to be called first.)";
        return filename;
    }
...
public:
    ...
    static iterator begin(bool filter(const Arguments&) = nullptr)
    {
        ...
        if(!ifs)
        {
            ...
            if(!ifs || ifs->fail())
            {
                rocsparselt_cerr << "Cannot open " << fileToOpen << ": " << strerror(errno)
                                 << std::endl;
                exit(EXIT_FAILURE);
            }
        }
