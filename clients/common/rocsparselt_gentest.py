...
def setdefaults(test):
    ...
    # Do not put constant defaults here -- use rocsparselt_common.yaml for that.
...
def write_signature(out):
    ...
    if 'signature_written' not in args:
        sig = 0
        byt = bytearray("rocSPARSELt", 'utf_8')
        byt.append(0)
        last_ofs = 0
        ...
        for i in range(0, ctypes.sizeof(param['Arguments']) - last_ofs):
            byt.append(0)
        byt.extend(bytes("ROCsparselT", 'utf_8'))
        byt.append(0)
        out.write(byt)
...
def generate(test, function):
    ...
    if 'rocsparselt_function' in test:
        func = test.pop('rocsparselt_function')
        if func in param['Functions']:
            test.update(param['Functions'][func])
        else:
            test['function'] = func.rpartition('rocsparselt_')[2]
        generate(test, function)
        return
